...
function Version31Modal({ config, row, language, relatedRows, onSave, onClose }: { config: PageConfig; row: Row | null; language: Language; relatedRows: Row[]; onSave: (row: Row) => void; onClose: () => void }) {
  ...
  const stackedPlanCell = (item: PostPlan31, index: number, topKey: keyof PostPlan31, topLabel: string, bottomKey: keyof PostPlan31, bottomLabel: string, className = "") => <td className={`plan-stacked-cell ${className}`}><div><small>{topLabel}</small>{renderPlanControl(item, index, topKey)}</div><div><small>{bottomLabel}</small>{renderPlanControl(item, index, bottomKey)}</div></td>;
  const planPaymentInfoColSpan = scheme === "C" ? 12 : 11;
  const planPaymentFields = (["reviewId", "strategist", "platform", "planningPostDate", "product", "yellowCart", "boostCode", "owning"] as const);
  const planPostInfoFields = (["postId", "postDate", "postLink"] as const);
  const planAdsInfoFields = (["postStatus", "sparkAdsStatus"] as const);
  const planTable = <div className="plan-table-wrap v31-plan-wrap"><table className={`plan-table v31-plan-table ${!isPayment ? "review-grouped-plan-table" : ""}`}><thead>{!isPayment && <tr className="plan-module-head"><th className="plan-module-payment" colSpan={planPaymentInfoColSpan}>Payment Info</th><th className="plan-module-post" colSpan={3}>Post Info</th><th className="plan-module-ads" colSpan={2}>Ads Info</th></tr>}<tr>{isPayment && <th className="plan-check-cell"><input type="checkbox" aria-label={label("全选发布计划", "Select all post plans", language)} checked={plans.length > 0 && selectedPlanRows.size === plans.length} onChange={toggleAllPlanRows} /></th>}{!isPayment && scheme === "C" && <th className="plan-payment-info-field plan-select-field" /> }<th className="plan-payment-info-field">No.</th><th className="plan-payment-info-field">Review ID</th><th className="plan-payment-info-field">KOL Strategist *</th><th className="plan-payment-info-field">Platform *</th><th className="plan-payment-info-field">Planning Post *</th><th className="plan-payment-info-field plan-stacked-header">Each Price * / Rate</th><th className="plan-payment-info-field plan-stacked-header">Content Type / Angle</th><th className="plan-payment-info-field">Product</th><th className="plan-payment-info-field">YC</th><th className="plan-payment-info-field">Boost Code</th><th className="plan-payment-info-field">Owning</th>{!isPayment && <><th className="plan-post-info-field plan-module-start">Post ID</th><th className="plan-post-info-field">Post Date</th><th className="plan-post-info-field">Post Link</th><th className="plan-ads-info-field plan-module-start">Review Status</th><th className="plan-ads-info-field">Spark Ads Status</th></>}</tr></thead><tbody>{plans.map((item, index) => <tr key={item.postNo} className={selectedPlan === item.postNo ? "selected-plan" : ""}>{isPayment && <td className="plan-check-cell"><input type="checkbox" aria-label={`${label("选择", "Select", language)} Post No. ${item.postNo}`} checked={selectedPlanRows.has(item.postNo)} onChange={() => togglePlanRow(item.postNo)} /></td>}{!isPayment && scheme === "C" && <td className="plan-payment-info-cell plan-select-field"><input type="radio" disabled={!reviewValue} checked={selectedPlan === item.postNo} onChange={() => setSelectedPlan(item.postNo)} /></td>}<td className="plan-payment-info-cell"><b>{item.postNo}</b></td>{planPaymentFields.slice(0, 4).map(key => <td className="plan-payment-info-cell" key={key}>{renderPlanControl(item, index, key)}</td>)}{stackedPlanCell(item, index, "eachPrice", "Each Price *", "rate", "Rate", "plan-payment-info-cell")}{stackedPlanCell(item, index, "contentType", "Content Type", "contentAngle", "Content Angle", "plan-payment-info-cell")}{planPaymentFields.slice(4).map(key => <td className="plan-payment-info-cell" key={key}>{renderPlanControl(item, index, key)}</td>)}{!isPayment && <>{planPostInfoFields.map((key, postIndex) => <td className={`plan-post-info-cell ${postIndex === 0 ? "plan-module-start" : ""}`} key={key}>{reviewInlineControl(item, index, key)}</td>)}{planAdsInfoFields.map((key, adsIndex) => <td className={`plan-ads-info-cell ${adsIndex === 0 ? "plan-module-start" : ""}`} key={key}>{reviewInlineControl(item, index, key)}</td>)}</>}</tr>)}</tbody></table></div>;
