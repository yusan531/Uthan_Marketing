...
type PostPlan31 = PostPlan & { strategist: string; reviewId: string; eachPrice: string; rate: string; product: string; sparkStatus: string; postId?: string; postDate?: string; postLink?: string; boostCodeValue?: string; expiredDate?: string; postStatus?: string; sparkAdsStatus?: string; addDate?: string; adsPic?: string; createdAfterApproval?: boolean };
...
function Version31Modal({ config, row, language, relatedRows, reviewRows, onSave, onClose, readOnly = false }: { config: PageConfig; row: Row | null; language: Language; relatedRows: Row[]; reviewRows: Row[]; onSave: (row: Row) => void; onClose: () => void; readOnly?: boolean }) {
  ...
  const [plans, setPlans] = useState<PostPlan31[]>(() => {
    ...
    const savedPlans = sourcePlans.map(item => {
      ...
      return { ...item, postId: reviewPlan.postId || review?.postId || item.postId, postDate: reviewPlan.postDate || review?.postDate || review?.actualPostDate || item.postDate, postLink: reviewPlan.postLink || review?.postLink || item.postLink, boostCodeValue: reviewPlan.boostCodeValue || review?.boostCodeValue || item.boostCodeValue, expiredDate: reviewPlan.expiredDate || review?.expiredDate || item.expiredDate, postStatus: reviewPlan.postStatus || review?.postStatus || item.postStatus, sparkAdsStatus: reviewPlan.sparkAdsStatus || review?.sparkAdsStatus || item.sparkAdsStatus, addDate: reviewPlan.addDate || review?.addDate || item.addDate, adsPic: reviewPlan.adsPic || review?.adsPic || item.adsPic } as PostPlan31;
    });
    return savedPlans.map(item => ({
      ...
      ...(Number(row?.postNo) === item.postNo ? { postId: String(row?.postId || ""), postDate: String(row?.postDate || row?.actualPostDate || ""), postLink: String(row?.postLink || ""), boostCodeValue: String(row?.boostCodeValue || item.boostCodeValue || ""), expiredDate: String(row?.expiredDate || item.expiredDate || ""), postStatus: normalizeReviewPostStatus(row?.postStatus || "Normal"), sparkAdsStatus: normalizeReviewSparkAdsStatus(row?.sparkAdsStatus || "None"), addDate: String(row?.addDate || ""), adsPic: String(row?.adsPic || "") } : {})
    }));
  });
  ...
  const copyPlan = () => {
    ...
    setPlans(current => [...current, { ...source, postNo: nextNo, reviewId: "", eachPrice: createdAfterApproval ? "0" : source.eachPrice, rate: createdAfterApproval ? "C" : source.rate, boostCode: "Yes", sparkStatus: "Yes", boostCodeValue: "", expiredDate: "", postId: "", postDate: "", postLink: "", postStatus: "Normal", sparkAdsStatus: "None", addDate: "", adsPic: "", createdAfterApproval }]);
    setSelectedPlan(nextNo);
    setSelectedPlanRows(new Set([nextNo]));
  };
  const deletePlan = () => {
  ...
  const updatePlan = (index: number, key: keyof PostPlan31, value: string) => setPlans(current => current.map((item, itemIndex) => {
    ...
    if (key === "postId") next.postDate = value.trim() ? item.postDate || new Date().toISOString().slice(0, 10) : "";
  ...
  const renderPlanControl = (item: PostPlan31, index: number, key: keyof PostPlan31) => {
    ...
    if (["reviewId", "rate", "sparkStatus"].includes(String(key))) return <input className="plan-system-field" value={String(item[key] ?? "")} readOnly placeholder={key === "reviewId" ? label("创建 Review 后自动生成", "Generated after Review is created", language) : label("系统自动带出", "Auto-filled", language)} title={key === "reviewId" ? label("Review 创建成功后生成并回写，不可编辑", "Generated and written back after Review creation; read-only", language) : label("系统字段，不可编辑", "System field; read-only", language)} />;
    ...
    return <input required={key === "planningPostDate" || key === "eachPrice"} type={key === "planningPostDate" ? "date" : key === "eachPrice" ? "number" : "text"} min={key === "eachPrice" ? 0 : undefined} value={String(item[key] ?? "")} onFocus={() => setSelectedPlan(item.postNo)} onChange={event => updatePlan(index, key, event.target.value)} />;
  };
  const reviewInlineControl = (item: PostPlan31, index: number, key: "postId" | "postDate" | "postLink" | "boostCodeValue" | "expiredDate" | "postStatus" | "sparkAdsStatus" | "addDate" | "adsPic") => {
    if (key === "postLink") {
    ...
    if (readOnly) {
      if (key === "postId") {
        const postId = String(item.postId || "").trim();
        const link = reviewPostLink(item);
        return postId && link ? <a className="plan-post-id-link" href={link} target="_blank" rel="noreferrer" title={label("打开帖子", "Open post", language)}>{postId}</a> : <span>{postId || "—"}</span>;
      }
    ...
    const postInfoField = key === "postId" || key === "boostCodeValue" || key === "expiredDate";
    if (postInfoField && row?.postInfoLocked) return <input className="plan-system-field" readOnly value={String(item[key] || "")} title={label("保存后不可修改", "Locked after save", language)} />;
    if (postInfoField) {
      const required = key === "boostCodeValue" ? Boolean(String(item.postId || "").trim()) : key === "expiredDate" ? Boolean(String(item.boostCodeValue || "").trim()) : false;
      return <input required={required} type={key === "expiredDate" ? "date" : "text"} value={String(item[key] || "")} onFocus={() => setSelectedPlan(item.postNo)} onChange={event => { setSelectedPlan(item.postNo); updatePlan(index, key, event.target.value); }} />;
    }
      ...
      <form onSubmit={submit} className="own-media-form">
        <div className="modal-scroll-area">
          ...
          <section className="own-media-section">
            ...
            <div className="own-media-summary-grid">{[["Post ID", form.postId], ["Post Date", form.postDate], ["Status", row ? "Active" : "-"], ["Update date", row?.updatedAt], ["Views (latest total)", row?.views], ["Days since actual post", "-"], ["Likes", row?.likes], ["Comments", row?.comments], ["Favorites", row?.favorites], ["Shares", row?.shares], ["Engagement rate", row?.engagementRate]].map(([name, value]) => <div key={String(name)}><span>{String(name)}</span><strong>{String(value || "-")}</strong></div>)}</div>
          </section>
          <section className="own-media-section">
...
function PostPlanCalendar({
  ...
  const calendarDimensionValue = (entry: Record<string, unknown>) => {
    ...
    const rawValue = dimension === "product"
      ? entry.product
      : dimension === "tier"
        ? entry.tier || entry.rate || entry.rateTier
        : dimension === "strategist"
          ? entry.owner
          : dimension === "specialist"
            ? entry.kolSpecialist || entry.specialist
            : dimension === "submitter"
              ? entry.submitter
              : entry.brand;
    return String(rawValue || label("未分配", "Unassigned", language));
  };
  ...
  const renderCell = (date: string, index: number) => {
    ...
    const dimensionGroups = Array.from(dayEntries.reduce((groups, item) => {
      const name = calendarDimensionValue(item.entry);
      groups.set(name, [...(groups.get(name) || []), item]);
      return groups;
    }, new Map<string, typeof monthEntries>()).entries());
    const dayCounts = dayEntries.reduce((counts, item) => {
      const status = postPlanScheduleStatus(item.entry, today, publishedPlanKeys);
      if (status === "completed" || status === "overdue-completed") counts.posted += 1;
      if (status === "overdue") counts.delayed += 1;
      if (status === "planned") counts.planned += 1;
      return counts;
    }, { posted: 0, delayed: 0, planned: 0 });
    const summaryItems = date < today
      ? [["posted", dayCounts.posted, label("Posted", "Posted", language)], ["delayed", dayCounts.delayed, label("Delayed", "Delayed", language)]] as const
      : date === today
...
function TargetDashboard({
  ...
  const basePaidPlanEntries = paidPayments.flatMap((payment, paymentIndex) => {
    ...
    const plans: Record<string, unknown>[] = savedPlans.length
      ? savedPlans
    ...
    return plans.map((plan, index) => {
      ...
      return {
        ...
        actualPostDate: review?.actualPostDate || review?.postDate || (plan as Record<string, unknown>).actualPostDate || (plan as Record<string, unknown>).postDate || "",
  ...
  const postedPlanEntries: Record<string, unknown>[] = [...reviewRows.filter((review) => {
...
export default function MarketingSystem() {
  ...
  function openReviewEditorFromCalendar(entry: Record<string, unknown>) {
    ...
    const fallback: Row = {
      ...
      expiredDate: String(entry.expiredDate || ""),
