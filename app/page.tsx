...
export default function MarketingSystem() {
  ...
  function PageHeader({ title, desc, action }: { title: string; desc: string; action?: ReactNode }) {
    return <div className="page-head"><div><p>{scope.toUpperCase()} · JUNE 2026</p><h1>{T(title)}</h1><span>{T(desc)}</span></div>{action}</div>;
  }

  function Dashboard() {
    return <><PageHeader title="Marketing 工作台" desc="从目标到发布结果，集中查看本月关键进展。" action={<button className="primary" onClick={() => setModal({ type: "campaign" })}>＋ {T("新建 Campaign")}</button>} />
      <div className="metrics"><article><span>Budget 使用率</span><b>{pct(spentBudget, totalBudget)}%</b><small>{money(spentBudget)} / {money(totalBudget)}</small><Progress value={pct(spentBudget, totalBudget)} /></article><article><span>Post MTD</span><b>{totalPosts}</b><small>目标 {targetPosts} · Pace {pct(totalPosts, targetPosts)}%</small><Progress value={pct(totalPosts, targetPosts)} tone="blue" /></article><article><span>GMV MTD</span><b>{money(totalGmv)}</b><small>较上月 ↑ 12.8%</small><Progress value={71} tone="green" /></article><article><span>合作中 KOL</span><b>{kols.filter(x => x.status === "合作中").length}</b><small>达人池共 {kols.length} 位</small><Progress value={68} tone="orange" /></article></div>
      <div className="two-col"><section className="card"><div className="card-head"><div><h2>目标达成</h2><p>Marketing 3.0 · Product Performance</p></div><button className="text-btn" onClick={() => setActive("target")}>查看全部 →</button></div><div className="product-list">{targets.map(x => <div className="product-line" key={x.id}><div><b>{x.product}</b><small>{x.priority} · {x.pic}</small></div><div className="inline-progress"><span>Post {x.postsMtd}/{x.posts}</span><Progress value={pct(x.postsMtd, x.posts)} tone={pct(x.postsMtd, x.posts) < 40 ? "orange" : "purple"} /></div><strong>{pct(x.gmvMtd, x.gmv)}% GMV</strong></div>)}</div></section>
      <section className="card"><div className="card-head"><div><h2>AI 管理摘要</h2><p>基于目标、预算和发布结果</p></div><span className="ai-badge">AI</span></div><div className="ai-summary"><div className="good"><b>表现领先</b><p>Serum Spray 发布达成 73%，GMV 转化领先。建议复用即时补水 Hook，并增加 A 级达人。</p></div><div className="warn"><b>需要跟进</b><p>Day Cream 发布仅达 22%，预算节奏同步偏慢。优先推动已收样达人在本周完成发布。</p></div><div><b>今日动作</b><p>审批 1 笔达人付款、确认 2 个待发样品、完成 3 条内容审核。</p></div></div></section></div>
  ...
  function PaymentPage() { return <><PageHeader title="Payment 付款中心" desc="提交、审批并跟踪达人和媒体付款。" action={<button className="primary" onClick={() => setModal({ type: "payment" })}>＋ 申请付款</button>} /><section className="card no-pad"><DataTable headers={["PAYEE", "ITEM", "OWNER", "AMOUNT", "STATUS", "ACTION"]}>{payments.map(x => <div className="tr" key={x.id}><div><b>{x.payee}</b></div><div>{x.item}</div><div>{x.owner}</div><div><b>{money(x.amount)}</b></div><div><span className={`status ${x.status}`}>{x.status}</span></div><div className="row-actions">{x.status === "待审批" ? <><button className="small-btn approve" onClick={() => { setPayments(v => v.map(r => r.id === x.id ? { ...r, status: "已批准" } : r)); notify("付款已批准"); }}>批准</button><button className="small-btn danger" onClick={() => { setPayments(v => v.map(r => r.id === x.id ? { ...r, status: "已拒绝" } : r)); notify("付款已拒绝"); }}>拒绝</button></> : <button className="small-btn" onClick={() => { setPayments(v => v.map(r => r.id === x.id ? { ...r, status: r.status === "已批准" ? "付款中" : r.status === "付款中" ? "已付款" : r.status } : r)); notify("付款状态已更新"); }}>推进</button>}</div></div>)}</DataTable></section></> }
  ...
  return <main className="app-shell">
    <aside className="sidebar"><div className="brand"><i>M</i><span>Marketing<strong>.</strong></span></div><div className="workspace"><i>G</i><span><b>Glowsicha Indonesia</b><small>{language === "en" ? "June 2026 · Day 17/30" : "2026年6月 · 第17/30天"}</small></span><em>⌄</em></div><p className="nav-title">WORKSPACE</p><nav>{menus.map(m => <button key={m.key} className={active === m.key ? "active" : ""} onClick={() => setActive(m.key)}><i>{m.icon}</i><span>{language === "en" ? m.labelEn : m.label}</span>{active === m.key && <em />}</button>)}</nav><footer><div className="profile"><i>MC</i><span><b>Mia Chen</b><small>{language === "en" ? "Brand Leader" : "品牌负责人"}</small></span><button>···</button></div></footer></aside>
    <section className="main-area"><header className="topbar"><div className="crumb"><span>Marketing</span><b>/</b><strong>{language === "en" ? activeMenu.labelEn : activeMenu.label}</strong></div><div className="top-controls"><select value={scope} onChange={e => setScope(e.target.value)}><option>GST Dashboard</option><option>Brand Dashboard</option><option>PIC Dashboard</option></select><label className="global-search">⌕<input value={search} onChange={e => setSearch(e.target.value)} placeholder={T("搜索当前模块...")} /></label><button className="switch" onClick={() => setLanguage(language === "zh" ? "en" : "zh")} title="中英文切换">{language === "zh" ? "EN" : "中"}</button><button className="switch theme-switch" onClick={() => setTheme(theme === "light" ? "dark" : "light")} title="亮色/暗黑切换">{theme === "light" ? "☾" : "☀"}</button><button className="bell">♧<i /></button><span className="avatar">M</span></div></header><div className="content">{renderPage()}</div></section>
