...
const emptyReviewPlan31 = (postNo = 1): PostPlan31 => ({ ...plan31Seed[0], postNo, strategist: "", reviewId: "", platform: "", contentType: "", contentAngle: "", planningPostDate: "", eachPrice: "", rate: "C", product: "", yellowCart: "", boostCode: "", owning: "", sparkStatus: "None" });
...
function Version31Modal({ config, row, language, relatedRows, onSave, onClose, readOnly = false }: { config: PageConfig; row: Row | null; language: Language; relatedRows: Row[]; onSave: (row: Row) => void; onClose: () => void; readOnly?: boolean }) {
  ...
  const changeReviewCreator = (value: string) => {
    setForm(current => ({ ...current, creatorName: value, paymentNo: "", reviewNo: "", brand: "", owner: "", supervisor: "", department: "", reviewPlatform: "TikTok", reviewContentType: "", reviewContentAngle: "", reviewProduct: "", reviewPlanningPost: "", reviewEachPrice: "", reviewRate: "C", reviewYellowCart: "No", reviewOwning: "Creator", reviewSparkStatus: "None", reviewBoostCode: "" }));
    setSelectedPlan(0);
  };
  const changeReviewPayment = (value: string) => {
    const payment = relatedRows.find(item => String(item.paymentNo || "") === value);
    const paymentPlans = value ? (Array.isArray(payment?.postPlans) ? payment.postPlans as PostPlan31[] : plan31Seed) : [emptyReviewPlan31()];
  ...
  const addPlan = () => {
    ...
    setPlans(current => [...current, { ...template, postNo: nextNo, reviewId: "", eachPrice: !isPayment && !paymentValue ? "" : eachPrice, rate: createdAfterApproval ? "C" : !isPayment && !paymentValue ? "C" : rateFromPrice(eachPrice), contentAngle: !isPayment && !paymentValue ? "" : "", planningPostDate: !isPayment && !paymentValue ? "" : "", boostCode: "", createdAfterApproval }]);
