...
type BatchPlanKey = "strategist" | "platform" | "contentType" | "contentAngle" | "planningPostDate" | "eachPrice" | "product" | "yellowCart" | "owning" | "addDate" | "adsPic";
...
function Version31Modal({ config, row, language, relatedRows, onSave, onClose, readOnly = false }: { config: PageConfig; row: Row | null; language: Language; relatedRows: Row[]; onSave: (row: Row) => void; onClose: () => void; readOnly?: boolean }) {
  ...
  const emptyBatchPlan: Record<BatchPlanKey, string> = { strategist: "", platform: "", contentType: "", contentAngle: "", planningPostDate: "", eachPrice: "", product: "", yellowCart: "", owning: "", addDate: "", adsPic: "" };
  ...
  const copyPlan = () => {
    ...
    setPlans(current => [...current, { ...source, postNo: nextNo, reviewId: "", eachPrice: createdAfterApproval ? "0" : source.eachPrice, rate: createdAfterApproval ? "C" : source.rate, boostCode: "", createdAfterApproval }]);
    setSelectedPlan(nextNo);
    if (isPayment) setSelectedPlanRows(new Set([nextNo]));
  };
  ...
  const reviewInlineControl = (item: PostPlan31, index: number, key: "postId" | "postDate" | "postLink" | "postStatus" | "sparkAdsStatus" | "addDate" | "adsPic") => {
    ...
    return <input type={key === "postDate" || key === "addDate" ? "date" : "text"} value={String(item[key] || "")} onFocus={() => setSelectedPlan(item.postNo)} onChange={event => { setSelectedPlan(item.postNo); updatePlan(index, key, event.target.value); }} />;
  };
  ...
  return <><Modal title={<span className="review-dialog-heading"><b>{isPayment ? (readOnly ? label("Payment3.1 查看", "Payment3.1 View", language) : "Payment3.1") : form.reviewNo ? "Review3.1-Edit" : "Review3.1-Add"}</b>{(isPayment || form.reviewNo) && <strong>{String(isPayment ? form.paymentNo : form.reviewNo)}</strong>}</span>} onClose={onClose} wide variant="v11-modal v31-modal">
    ...
    <label className="form-field"><span>Planning Post</span><input type="date" value={batchPlanValues.planningPostDate} onChange={event => updateBatchPlanValue("planningPostDate", event.target.value)} /></label><label className="form-field"><span>Each Price</span><input type="number" min="0" placeholder={label("留空则不修改", "Blank keeps original", language)} value={batchPlanValues.eachPrice} onChange={event => updateBatchPlanValue("eachPrice", event.target.value)} /></label>{!isPayment && <><label className="form-field"><span>Add Date</span><input type="date" value={batchPlanValues.addDate} onChange={event => updateBatchPlanValue("addDate", event.target.value)} /></label><label className="form-field"><span>Ads PIC</span><input value={batchPlanValues.adsPic} placeholder={label("输入负责人", "Enter Ads PIC", language)} onChange={event => updateBatchPlanValue("adsPic", event.target.value)} /></label></>}
