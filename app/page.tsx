...
function Version31Modal({ config, row, language, onSave, onClose }: { config: PageConfig; row: Row | null; language: Language; onSave: (row: Row) => void; onClose: () => void }) {
  ...
  return <><Modal title={<span className="review-dialog-heading"><b>{isPayment ? "Payment3.1" : `Review3.1-${scheme}`}</b><strong>{String(isPayment ? form.paymentNo : (form.reviewNo || label("新建 Review", "New Review", language)))}</strong><em>{isPayment ? label("主单 + Post Plan", "Payment + Post Plan", language) : label(`方案 ${scheme}`, `Option ${scheme}`, language)}</em></span>} onClose={onClose} wide variant="v11-modal v31-modal">
    <form onSubmit={save} className="v11-form v31-form"><div className="modal-scroll-area">
      ...
      {!isPayment && scheme === "A" && paymentValue && <><div className={`single-review-plan-heading ${reviewValue ? "planned" : "unplanned"}`}><div><span>{label("当前模式", "Current Mode", language)}</span><b>{reviewValue ? label("计划内 Review", "Planned Review", language) : label("计划外 Review", "Unplanned Review", language)}</b></div><small>{reviewValue ? label("计划字段从所选 Review 带出；本页一次只修改这一条 Review。", "Plan fields come from the selected Review; this page edits one Review only.", language) : label("未选择 Review；保存时将创建一条计划外 Review。", "No Review selected; saving creates one unplanned Review.", language)}</small></div>{section("Post Plan")}<div className="v11-grid single-review-plan-grid"><label className="form-field"><span>Platform</span><input value={String(form.reviewPlatform || "")} readOnly /></label><label className="form-field"><span>Content Type</span><select value={String(form.reviewContentType || "")} onChange={event => updateSingleReviewPlan("reviewContentType", event.target.value)}><option value="">{label("请选择", "Select", language)}</option>{(planOptions.contentType || []).map(option => <option key={option}>{option}</option>)}</select></label><label className="form-field"><span>Content Angles</span><select value={String(form.reviewContentAngle || "")} onChange={event => updateSingleReviewPlan("reviewContentAngle", event.target.value)}><option value="">{label("请选择", "Select", language)}</option>{(planOptions.contentAngle || []).map(option => <option key={option}>{option}</option>)}</select></label><label className="form-field"><span>Product</span><select value={String(form.reviewProduct || "")} onChange={event => updateSingleReviewPlan("reviewProduct", event.target.value)}><option value="">{label("请选择", "Select", language)}</option>{(planOptions.product || []).map(option => <option key={option}>{option}</option>)}</select></label><label className="form-field"><span>Planning Post</span><input type="date" value={String(form.reviewPlanningPost || "")} readOnly={Boolean(reviewValue)} onChange={event => updateSingleReviewPlan("reviewPlanningPost", event.target.value)} /></label><label className="form-field"><span>Each Price</span><input type="number" min="0" value={String(form.reviewEachPrice || "")} readOnly={Boolean(reviewValue)} onChange={event => updateSingleReviewPlan("reviewEachPrice", event.target.value)} /></label><label className="form-field"><span>Rate</span><input value={String(form.reviewRate || "")} readOnly /></label><label className="form-field"><span>Yellow Cart</span><select value={String(form.reviewYellowCart || "")} onChange={event => updateSingleReviewPlan("reviewYellowCart", event.target.value)}>{(planOptions.yellowCart || []).map(option => <option key={option}>{option}</option>)}</select></label><label className="form-field"><span>Owning</span><select value={String(form.reviewOwning || "")} onChange={event => updateSingleReviewPlan("reviewOwning", event.target.value)}>{(planOptions.owning || []).map(option => <option key={option}>{option}</option>)}</select></label><label className="form-field"><span>Spark Code Status</span><select value={String(form.reviewSparkStatus || "")} onChange={event => updateSingleReviewPlan("reviewSparkStatus", event.target.value)}>{["None","Required","Provided","Expired"].map(option => <option key={option}>{option}</option>)}</select></label><label className="form-field"><span>Boost Code</span><input value={String(form.reviewBoostCode || "")} onChange={event => updateSingleReviewPlan("reviewBoostCode", event.target.value)} /></label></div></>}
