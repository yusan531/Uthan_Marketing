...
function Version31Modal({ config, row, language, relatedRows, onSave, onClose, readOnly = false }: { config: PageConfig; row: Row | null; language: Language; relatedRows: Row[]; onSave: (row: Row) => void; onClose: () => void; readOnly?: boolean }) {
  ...
  const addPlan = () => {
    ...
    setPlans(current => [...current, { ...plan31Seed[0], postNo: nextNo, reviewId: "", eachPrice, rate: createdAfterApproval ? "C" : rateFromPrice(eachPrice), contentAngle: "", planningPostDate: "", boostCode: "", createdAfterApproval }]);
  ...
  const section = (title: string) => <div className={`form-section-title ${!isPayment && title === "Post Plan" ? "review-plan-section-title" : ""}`}><span className="form-section-heading"><i />{title}</span>{!isPayment && title === "Post Plan" && <><div className="review-plan-jump" aria-label={label("快速定位表格模块", "Jump to table module", language)}>{(["payment", "post", "ads"] as const).map(module => <button type="button" key={module} className={reviewPlanModule === module ? "active" : ""} onClick={() => jumpToReviewPlanModule(module)}>{module === "payment" ? "Payment Info" : module === "post" ? "Post Info" : "Ads Info"}</button>)}</div>{!readOnly && scheme === "B" && <div className="review-plan-actions"><button type="button" className="button secondary" disabled={selectedPlanRows.size === 0} onClick={openBatchPlanEdit}><Edit3 size={14}/>{label("批量修改", "Batch Edit", language)}</button><button type="button" className="button primary" onClick={addPlan}><Plus size={14}/>{label("新建", "New", language)}</button><button type="button" className="button ghost" disabled={plans.length === 0} onClick={copyPlan}><ClipboardList size={14}/>{label("复制", "Copy", language)}</button></div>}</>}</div>;
    ...
    setPlans(current => {
      if (nextQuantity >= current.length) {
        return [...current, ...Array.from({ length: nextQuantity - current.length }, (_, index) => ({
          ...
          eachPrice: String(form.unitPrice || plan31Seed[0].eachPrice),
          rate: rateFromPrice(String(form.unitPrice || plan31Seed[0].eachPrice)),
          reviewId: "",
          contentAngle: "",
          planningPostDate: "",
          boostCode: "",
  ...
  const planCell = (item: PostPlan31, index: number, key: keyof PostPlan31, className = "") => <td className={`plan-payment-info-cell ${className}`}>{renderPlanControl(item, index, key)}</td>;
  const reviewCell = (item: PostPlan31, index: number, key: "postId" | "postDate" | "postLink" | "postStatus" | "sparkAdsStatus" | "addDate" | "adsPic", className = "") => <td className={className}>{reviewInlineControl(item, index, key)}</td>;
  const planPaymentInfoColSpan = scheme === "C" ? 16 : 15;
  const planPaymentFields = (["reviewId", "strategist", "platform", "planningPostDate"] as const);
  const planTable = <div className="plan-table-wrap v31-plan-wrap"><table className={`plan-table v31-plan-table ${!isPayment ? "review-grouped-plan-table" : ""}`}><thead>{!isPayment && <tr className="plan-module-head"><th className="plan-module-payment" colSpan={planPaymentInfoColSpan}>Payment Info</th><th className="plan-module-post" colSpan={3}>Post Info</th><th className="plan-module-ads" colSpan={4}>Ads Info</th></tr>}<tr><th className="plan-check-cell"><input type="checkbox" disabled={readOnly} aria-label={label("全选发布计划", "Select all post plans", language)} checked={plans.length > 0 && selectedPlanRows.size === plans.length} onChange={toggleAllPlanRows} /></th>{!isPayment && scheme === "C" && <th className="plan-payment-info-field plan-select-field" /> }<th className="plan-payment-info-field">No.</th><th className="plan-payment-info-field">Review ID</th><th className="plan-payment-info-field">KOL Strategist *</th><th className="plan-payment-info-field">Platform *</th><th className="plan-payment-info-field">Planning Post *</th><th className="plan-payment-info-field">Each Price *</th><th className="plan-payment-info-field">Rate</th><th className="plan-payment-info-field">Content Type</th><th className="plan-payment-info-field">Content Angle</th><th className="plan-payment-info-field plan-product-header">Product</th><th className="plan-payment-info-field">Yellow Cart</th><th className="plan-payment-info-field">Owning</th><th className="plan-payment-info-field">Boost Code Status</th><th className="plan-payment-info-field">Boost Code</th>{!isPayment && <><th className="plan-post-info-field plan-module-start">Post ID</th><th className="plan-post-info-field">Post Date</th><th className="plan-post-info-field">Post Link</th><th className="plan-ads-info-field plan-module-start">Review Status</th><th className="plan-ads-info-field">Spark Ads Status</th><th className="plan-ads-info-field">Ad Date</th><th className="plan-ads-info-field">Ads PIC</th></>}</tr></thead><tbody>{plans.map((item, index) => <tr key={item.postNo} className={selectedPlan === item.postNo ? "selected-plan" : ""}><td className="plan-check-cell"><input type="checkbox" disabled={readOnly} aria-label={`${label("选择", "Select", language)} Post No. ${item.postNo}`} checked={selectedPlanRows.has(item.postNo)} onChange={() => togglePlanRow(item.postNo)} /></td>{!isPayment && scheme === "C" && <td className="plan-payment-info-cell plan-select-field"><input type="radio" disabled={readOnly || !reviewValue} checked={selectedPlan === item.postNo} onChange={() => setSelectedPlan(item.postNo)} /></td>}<td className="plan-payment-info-cell"><b>{item.postNo}</b></td>{planPaymentFields.map(key => <td className="plan-payment-info-cell" key={key}>{renderPlanControl(item, index, key)}</td>)}{planCell(item, index, "eachPrice")}{planCell(item, index, "rate")}{planCell(item, index, "contentType")}{planCell(item, index, "contentAngle")}{planCell(item, index, "product", "plan-product-cell")}{planCell(item, index, "yellowCart")}{planCell(item, index, "owning")}{planCell(item, index, "sparkStatus")}{planCell(item, index, "boostCode")}{!isPayment && <>{reviewCell(item, index, "postId", "plan-post-info-cell plan-module-start")}{reviewCell(item, index, "postDate", "plan-post-info-cell")}{reviewCell(item, index, "postLink", "plan-post-info-cell plan-post-link-cell")}{reviewCell(item, index, "postStatus", "plan-ads-info-cell plan-module-start")}{reviewCell(item, index, "sparkAdsStatus", "plan-ads-info-cell")}{reviewCell(item, index, "addDate", "plan-ads-info-cell")}{reviewCell(item, index, "adsPic", "plan-ads-info-cell")}</>}</tr>)}</tbody></table></div>;
  const postPlanSection = <>{section("Post Plan")}{isPayment && !readOnly && <div className="post-plan-title"><div><strong>{label("付款内发布计划", "Payment Post Plan", language)}</strong></div><div className="v31-plan-actions"><button type="button" className="button secondary" disabled={selectedPlanRows.size === 0 || isPaymentLocked} onClick={openBatchPlanEdit}><Edit3 size={14}/>{label("批量修改", "Batch Edit", language)}</button><button type="button" className="button primary" onClick={addPlan}><Plus size={14}/>{label("新增", "Add", language)}</button><button type="button" className="button ghost" onClick={copyPlan}><ClipboardList size={14}/>{label("复制", "Copy", language)}</button></div></div>}{!isPayment && !paymentValue ? <div className="review-plan-locked"><LockKeyholeOpen size={18}/><b>{label("请先选择 Creator，再选择关联的 Payment", "Select a Creator, then the linked Payment", language)}</b></div> : planTable}</>;
  ...
  return <><Modal title={<span className="review-dialog-heading"><b>{isPayment ? (readOnly ? label("Payment3.1 查看", "Payment3.1 View", language) : "Payment3.1") : form.reviewNo ? "Review3.1-Edit" : "Review3.1-Add"}</b>{(isPayment || form.reviewNo) && <strong>{String(isPayment ? form.paymentNo : form.reviewNo)}</strong>}</span>} onClose={onClose} wide variant="v11-modal v31-modal">
    <form onSubmit={save} className={`v11-form v31-form ${!isPayment && row ? "review-edit-mode" : ""}`}><div className="modal-scroll-area">
      {section(label("基础信息", "Base Info", language))}{isPayment ? <div className="v11-grid">{field("paymentNo", "Payment ID", "text", true)}{field("country", "Country", "text", true)}{field("creatorName", "Creator Name")}{field("brand", "Brand", "text", true)}{field("owner", "KOL Strategist")}{field("supervisor", "Supervisor")}{field("submitter", "Submitter", "text", true)}{field("department", "Initiator Department", "text", true)}<label className="form-field"><span>Quantity</span><input type="number" min="0" value={plans.length} readOnly={readOnly || isPaymentLocked} onChange={event => updateQuantity(event.target.value)} /></label><label className="form-field"><span>Each Price</span><input type="number" min="0" value={String(form.unitPrice || "")} readOnly={readOnly || isPaymentLocked} onChange={event => updateBasePrice(event.target.value)} /></label><label className="form-field"><span>Total Price</span><input value={planTotal} readOnly /></label><label className="form-field"><span>Expected Finish All Post Date</span><input value={planExpectedFinish} readOnly /></label></div> : <div className="v11-grid review-link-grid"><label className="form-field"><span>Country</span><select required disabled={readOnly} value={String(form.country || "")} onChange={event => set("country", event.target.value)}><option value="">{label("请选择", "Select", language)}</option>{["ID", "MY", "VN", "TH", "PH", "SG", "MX"].map(value => <option key={value}>{value}</option>)}</select></label><label className="form-field"><span>Creator Name *</span><select required disabled={readOnly} value={creatorValue} onChange={event => changeReviewCreator(event.target.value)}><option value="">{label("请选择 Creator", "Select Creator", language)}</option>{availableCreators.map(creator => <option key={creator}>{creator}</option>)}</select></label><label className="form-field"><span>Payment ID *</span><select required disabled={readOnly || !creatorValue} value={paymentValue} onChange={event => changeReviewPayment(event.target.value)}><option value="">{creatorValue ? label("请选择关联 Payment", "Select linked Payment", language) : label("请先选择 Creator", "Select Creator first", language)}</option>{availablePayments.map(payment => <option key={payment}>{payment}</option>)}</select></label><label className="form-field"><span>{label("Review ID（可选）", "Review ID (Optional)", language)}</span><select disabled={readOnly || !paymentValue} value={reviewValue} onChange={event => changeReviewLink(event.target.value)}><option value="">{paymentValue ? label("不选择（保存时新建计划外 Review）", "None (create an unplanned Review on save)", language) : label("请先选择 Payment", "Select Payment first", language)}</option>{availableReviews.map(review => <option key={review}>{review}</option>)}</select></label><label className="form-field"><span>Brand</span><input value={String(form.brand || "")} readOnly /></label><label className="form-field"><span>KOL Strategist</span><input value={String(form.owner || "")} readOnly /></label><label className="form-field"><span>Submitter</span><input value={String(form.submitter || "")} readOnly /></label><label className="form-field"><span>Initiator Department</span><input value={String(form.department || "")} readOnly /></label></div>}
      {postPlanSection}
